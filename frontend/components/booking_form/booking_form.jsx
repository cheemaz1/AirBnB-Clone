...
class CreateBooking extends React.Component {
    ...
    setAlreadyBooked() {
        ...
        this.props.bookedDates.forEach(dateSet => {
            ...
            for (let i = 0; i < diff - 1; i++) {
                let temp = `${first.getFullYear()}/${first.getMonth() + 1}/${first.getDate() + 1
                    }`;
    ...
    handleSubmit(e) {
        ...
        if (this.props.currentUserId) {
            let begin = `${this.state.check_in.getMonth()}/${this.state.check_in.getDate()}/${this.state.check_in.getFullYear()}`;
            let end = `${this.state.check_out.getMonth()}/${this.state.check_out.getDate()}/${this.state.check_out.getFullYear()}`;
            if (
                ...
                this.props
                    ...
                    .then(res =>
                        this.props.history.push(
                            `/home/${res.booking.home_id}/booking/${res.booking.id}`
                        )
                    );
                this.errors = {};
            }
    ...
    setTotalCost() {
        let days =
            ...
            (1000 * 3600 * 24);
        this.setState({ price: (days * this.props.homePrice).toFixed(2) });
    }
